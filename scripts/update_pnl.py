...
CATEGORY_ROWS = {
    ...
    'Расходы на оборудование':   26,
    'Покупка инвентаря':         27,
    'Упаковка':                  28,
    'Расходы на логистику':      29,
    'Выплаты Beeyor':            30,  # агрегатор / комиссия доставки
    'Выплаты Teztar':            30,  # агрегатор Teztar
    'Маркетинг':                 31,
    'Poster':                    32,  # CRM
    'Интернет':                  33,
    'Хозяйственные расходы':     34,
    'Прочие расходы':            35,
    'Документы для заведения':   36,
    'Форс - мажор':              37,  # ЗБ
    'Форс мажор':                37,  # ОВИР
    'Налоги':                    40,
    'Погашение долгов':          44,
    'Инвестиции':                45,
    'Выплаты дивидентов':        46,  # опечатка в Poster
    'Выплаты дивидендов':        46,
}
...
def run(year=None, month_from=None, month_to=None):
    ...
    for month in range(month_from, month_to + 1):
        ...
        for loc in LOCATIONS:
            ...
            rev  = data.get(2, 0)
            cogs = data.get(4, 0) + data.get(5, 0) + data.get(7, 0) + data.get(8, 0)
            opex = sum(v for k, v in data.items() if 18 <= k <= 37)
