...
EXP_STRUCT=[
 ('block','Итого — Обязательное / Быт', ['Дом','Машина','Подписки','Связь','Рассрочка','Семья']),
 ('block','Итого — Еда', ['Продукты','Кафе']),
 ('block','Итого — Здоровье / Тело', ['Лечение / Медикаменты','Спортивное питание','Абонемент в зал','БАДы','Массаж / Сауна','Уход за собой']),
 ('block','Итого — Хотелки / Досуг', ['Одежда/Обувь','Гаджеты','Подарки','Развлечение','Свидания','Путешествие','Обучение','Курение']),
 ('single',None,['Прочее']),
]
...
if DASH in ids:
    for rrow in api('get',f'/values/{DASH}!A1:B120?valueRenderOption=UNFORMATTED_VALUE').get('values',[]):
        ...
        if rrow and str(rrow[0]).strip() and len(rrow)>1 and isinstance(rrow[1],(int,float)):
            prev[rrow[0]]= rrow[1]
# План по категориям — из «Лимитов», и они главнее записанного в дашборде.
# 14.09.2026 выяснилось, почему иначе нельзя: лимиты поменяли на новые
# нормы, а дашборд продолжал показывать старые 9 931 — он подставлял
# собственное прежнее значение. Два источника плана означают, что один
# из них всегда врёт. Источник один — лист «Лимиты»; прежнее значение
# остаётся только у категорий, которых в «Лимитах» нет вовсе.
limplan={}
...
for c in exp_order:
    ...
    d(c, limplan.get(c, prev.get(c,'')), f'=INDEX(PnL!B{rr}:M{rr},1,$B$2)', f'=B{row}-C{row}',
