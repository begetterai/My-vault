...
def build():
    ...
    def reg(cl):
        ...
        if not d.get('code'):
            return '—'
        t = f"{d['code']} · {d.get('title', '')}"
        return (f"<a href=\"{d['url']}\">{t}</a>" if d.get('url')
                else f'{t} <b>— ссылки нет</b>')

    qz = [[cl['title'],
           ', '.join(cl['dept']) if isinstance(cl.get('dept'), list)
           else (cl.get('dept') or 'всем'),
           str(len(cl.get('questions', []))), reg(cl), '']
...
открыто только обучение. Колонка «Регламент» — документ, который тренинг
объясняет; из приложения он открывается по ссылке.</p>
""" + tbl(['Тренинг', 'Кому', 'Вопросов', 'Регламент', 'Правка'],
          qz, ['28%', '18%', '9%', '28%', None]) + """
<p>Сколько сдаёт новичок до выхода на станцию: кухня — 13 тренингов
(~82 мин), цех — 12, бар — 10, касса — 8, зал — 6. Руководитель видит
все 16.</p>
""" + f"""
