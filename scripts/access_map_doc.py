...
<h2>10. Решено 24.08.2026</h2>
""" + tbl(['Вопрос', 'Решение'],
          [['Бланки у линейных позиций',
            '<b>Оставляем как есть.</b> У каждой позиции один бланк, '
            'остальные закреплены за руководителем'],
           ['«Инвентаризация: проведение»',
            '<b>Стала отметкой, а не чек-листом.</b> Записывается: кто считал, '
            'что посчитано, во сколько начали, кому передан лист, замечания. '
            'Счёт остаётся на бумаге'],
           ['Санитарный чек-лист',
            '<b>Ежедневно до 9:30.</b> Переехал из «по необходимости» '
            'в «Сегодня»'],
           ['Тренинг открывает бланк',
            '<b>Оставляем две связи:</b> списание и приём поставки']],
          ['30%', None]) + """

<h2>11. Режим обкатки</h2>
<p>С 24.08.2026 в приложении трое: директор, управляющий ЗБ, старший смены ЗБ,
управляющий ОВИР. Работаем на живых данных.</p>
<ul>
<li><b>Данные обкатки не удаляются по ходу</b> — по ним видно, где механика
спотыкается. Полная очистка один раз, перед официальным запуском.</li>
<li><b>Выключено на обкатку:</b> сводки в боте, напоминания о просрочке.
Дедлайны при этом работают — по ним группируется «Сегодня».</li>
<li><b>Доступ по ролям</b> настраивается на сервере, без выката кода.</li>
</ul>
"""
    return b
