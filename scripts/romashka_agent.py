...
BUDGET_CATS = {'Дом','Машина','Гаджеты','Подписки','Кафе','Продукты','Здоровье / Тело','Курение','Одежда/Обувь','Развлечение','Обучение','Семья','Подарки','Оплата кредита','Путешествие','Прочее'}
...
TOOLS_SPEC = [
 ...
 {'type':'function','function':{'name':'add_budget_entry','description':'Записать личный доход/расход в бюджет-ПНЛ. Если в сообщении несколько трат — вызови для каждой отдельно. Категорию бери СТРОГО из enum. Правила: аренда/коммуналка/дом→Дом; бензин/ремонт авто/мойка→Машина; покупка или ремонт техники/наушников/чехол/ламинат/стекло/аксессуар→Гаджеты; ОПЛАТА телефона/интернета/связь/Claude/любая подписка→Подписки; кафе/ресторан/кофе→Кафе; продукты/базар→Продукты; врач/аптека/зубы/зал/спорт/уход→Здоровье / Тело; сигареты→Курение; одежда/обувь→Одежда/Обувь; кино/отдых/свидание/хобби→Развлечение; курс/книга/учёба→Обучение; родители/помощь родным→Семья; подарок→Подарки; погашение кредита→Оплата кредита; поездка/билеты/отель→Путешествие; непонятно→Прочее. Доход: зарплата→Зарплата, иной приход→Прочий доход. ВСЕГДА заполняй comment — краткое описание траты из сообщения (что купил/на что), напр. «кофе 50 в старбаксе»→comment="кофе в старбаксе"; «продукты 800»→comment="продукты". Если в сообщении вообще нет описания (только сумма) — оставь comment пустым.',
   'parameters':{'type':'object','properties':{
     'amount':{'type':'string','description':'сумма числом, например "160"'},'category':{'type':'string','enum':['Дом','Машина','Гаджеты','Подписки','Кафе','Продукты','Здоровье / Тело','Курение','Одежда/Обувь','Развлечение','Обучение','Семья','Подарки','Оплата кредита','Путешествие','Прочее','Зарплата','Прочий доход']},
