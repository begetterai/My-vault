...
BUDGET_CATS = {'Дом','Машина','Гаджеты','Подписки','Кафе','Продукты','Лечение / Медикаменты','Спортивное питание','Абонемент в зал','БАДы','Массаж / Сауна','Курение','Одежда/Обувь','Развлечение','Обучение','Семья','Подарки','Оплата кредита','Путешествие','Прочее'}
...
TOOLS_SPEC = [
 ...
 {'type':'function','function':{'name':'add_budget_entry','description':'Записать личный доход/расход в бюджет-ПНЛ. Если в сообщении несколько трат — вызови для каждой отдельно. Категорию бери СТРОГО из enum. Правила: аренда/коммуналка/дом→Дом; бензин/ремонт авто/мойка→Машина; покупка или ремонт техники/наушников/чехол/ламинат/стекло/аксессуар→Гаджеты; ОПЛАТА телефона/интернета/связь/Claude/любая подписка→Подписки; кафе/ресторан/кофе→Кафе; продукты/базар→Продукты; врач/аптека/зубы/лечение→Лечение / Медикаменты; протеин/спортпит→Спортивное питание; фитнес/зал/абонемент→Абонемент в зал; витамины/добавки→БАДы; массаж/сауна/баня→Массаж / Сауна; сигареты→Курение; одежда/обувь→Одежда/Обувь; кино/отдых/свидание/хобби→Развлечение; курс/книга/учёба→Обучение; родители/помощь родным→Семья; подарок→Подарки; погашение кредита→Оплата кредита; поездка/билеты/отель→Путешествие; непонятно→Прочее. Накопления (тип «накопление», НЕ расход): отложил/накопил/в подушку/на чёрный день→Накопления / Подушка; инвестировал/акции/крипто/вклад→Инвестиции. Доход: зарплата→Зарплата, иной приход→Прочий доход. ВСЕГДА заполняй comment — краткое описание траты из сообщения (что купил/на что), напр. «кофе 50 в старбаксе»→comment="кофе в старбаксе"; «продукты 800»→comment="продукты". Если в сообщении вообще нет описания (только сумма) — оставь comment пустым.',
   'parameters':{'type':'object','properties':{
     'amount':{'type':'string','description':'сумма числом, например "160"'},'category':{'type':'string','enum':['Дом','Машина','Гаджеты','Подписки','Кафе','Продукты','Лечение / Медикаменты','Спортивное питание','Абонемент в зал','БАДы','Массаж / Сауна','Курение','Одежда/Обувь','Развлечение','Обучение','Семья','Подарки','Оплата кредита','Путешествие','Прочее','Накопления / Подушка','Инвестиции','Зарплата','Прочий доход']},
     'kind':{'type':'string','enum':['расход','доход','накопление']},'comment':{'type':'string'},'date':{'type':'string','description':'YYYY-MM-DD, если операция задним числом'}},'required':['amount','comment']}}},
...
def describe_action(fn, args):
    ...
    if fn=='add_budget_entry':
        c0 = args.get('category','Прочее')
        if str(args.get('kind','расход')).lower().startswith('нак') or c0 in SAVINGS_CATS:
            k='Накопление'; c = c0 if c0 in SAVINGS_CATS else 'Накопления / Подушка'
        elif str(args.get('kind','расход')).lower().startswith('дох'):
