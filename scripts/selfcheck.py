...
def check_rules_docs():
    ...
    # Событие убрали из RULES, а строку здесь забыли — раньше это роняло
    # проверку целиком, и все остальные события переставали сверяться молча.
    # Теперь говорим об этом, а сверку доводим до конца.
    for ev in [e for e in WHERE if e not in SC.RULES]:
        warn.append(f'«{WHERE.pop(ev)}» больше нет в RULES — убери строку '
                    f'из документов')
    # Обратная сторона: событие добавили в код, а описать людям забыли.
    # «Опоздание» и «быстрее норматива» описаны словами, без суммы рядом.
    for ev in SC.RULES:
        if ev not in WHERE and ev not in ('late', 'too_fast'):
            warn.append(f'событие «{ev}» есть в коде, но не сверяется '
                        f'с документами')
