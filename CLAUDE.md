...
Набор из [mattpocock/skills](https://github.com/mattpocock/skills), отобрано восемь.
Вызываются командой со слэшем:

| Команда | Что делает |
|---------|------------|
| `/grill-me` | Прожарка: допрос по плану или гипотезе, пока не останется слабых мест |
| `/wait-what` | «Стоп, не понял» — объяснить то же самое заново и проще |
| `/to-questionnaire` | Превратить непонятное в анкету для того, кто знает |
| `/handoff` | Свернуть разговор в документ передачи для следующей сессии |
| `/teach` | Научить теме, с сохранением прогресса между сессиями |
| `/research` | Разобраться в вопросе по первоисточникам, вывод — заметкой |
| `/diagnosing-bugs` | Разбор трудной ошибки по шагам |
| `/grilling` | То же, что `/grill-me` — вызывается и по смыслу разговора |

Описания переведены, ссылки на чужие файлы (`CONTEXT.md`, issue-трекер) убраны.
При обновлении из оригинала правки придётся наложить заново.
