...
# и сообщения в консоль браузера — их человек не видит.
OWN = {'Ромашка', 'РУ', 'ТҶ', 'Забон / Язык', 'офлайн-режим не включился:',
       'словарь не загрузился:', 'перевод:', 'язык:'}

# Слова, которые на таджикском пишутся так же. Они переведены — просто
# перевод совпал с оригиналом.
#
# В словарь их класть НЕЛЬЗЯ: пара, где значение равно ключу, заставляла
# приложение переписывать один и тот же текст по кругу, и 13.09 оно от
# этого зависало при переключении языка. В коде страницы стоит защита,
# но держать такие пары в файле всё равно незачем — поэтому список здесь.
SAME = {'Логин', 'Касса', 'Бар', 'Смена', 'смена', 'Чек-лист', 'Директор',
        'Витрина', 'Кофемашина', 'Фритюрница', 'Пароконвектомат',
        'кг', 'г', 'л', 'мл', 'сом', 'индекс', '· радиус', '— логин',
        'Минус 12', 'Минус 50', '2. Санитария', '100 г', '1 бокс = 3,8 л',
        'ЗБ 00:30 · ОВИР 03:30'}
...
def skip(s):
    ...
    return (s in TABS or s in PARTS or s in OWN or s in SAME
