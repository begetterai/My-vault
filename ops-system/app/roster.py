#!/usr/bin/env python3
"""Наряд на завтра: кто выходит, на какую позицию, во сколько.
...
Почему наряд, а не постоянный график: выходные у сети — это когда кто-то
кого-то заменяет. Постоянное расписание в такой схеме устаревает в первый же
день, а вечерний наряд всегда описывает реальность.
...
START = {'цех': '07:00', 'кухня': '09:00', 'касса': '09:30',
         'бар': '09:30', 'зал': '08:30', 'доставка': '10:00'}
...
def planned(day, point=None):
    """Кто по наряду выходит в этот день."""
    return rows(day, point)


def for_person(day, who):
    """Строка наряда конкретного человека или None."""
    return next((r for r in rows(day) if r['who'] == who), None)


def dept_of(day, who, fallback=''):
    """Позиция на день: наряд главнее того, что записано в «Команде».
...
def last_roster(point, before):
    """Наряд последнего дня, когда он был. По нему делается черновик."""
    days = {}
...
def template(point, day):
    """Черновик наряда: состав предыдущего дня, а если его нет — команда точки.

    Черновик — это не догма, а способ не набирать одно и то же вручную:
    иначе управляющий начнёт отправлять наряд не глядя.
    """
...
def save(day, point, people, author):
    """Записать наряд на день. Повторная отправка заменяет прежний.
    ...
    for p in people:
        ...
        # Один человек — одна строка на день: иначе наряд врёт про состав
        # смены, а с ним врут явка, опоздания и закрытый день.
...
def _rewrite(day, point, new_rows):
    """Наряд дня переписывается целиком: старые строки этого дня гасим."""
    ds = day_str(day)
...
def summary(day, point=None):
    """Сводка по наряду: кто подтвердил, кто нет, кто отказался."""
    rs = rows(day, point)
    if not rs:
        return None
