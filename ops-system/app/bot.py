...
ROLES = ['Управляющий', 'Старший смены', 'Повар',
         'Заготовщик — лаваши', 'Заготовщик — мясо и соусы',
         'Заготовщик — выпечка и десерты',
         'Бариста', 'Кассир', 'Уборщица', 'Закупщик']
...
# Роль в «Команде» человеческая, а отдел — то, по чему открываются чек-листы.
# Раньше бот отдел не заполнял, и добавленный человек входил в пустой экран:
# листы позиции ему не показывались, потому что позиции у него не было.
# «Заготовщик» появился 30.08: роли цеха не было вовсе, и добавить
# человека в цех было нельзя — Владимир упёрся в это при заведении людей.
# Зону цеха (лаваши · мясо и соусы · выпечка и десерты) человек выбирает
# в приложении как рабочее место, как повар выбирает станцию кухни.
DEPT_BY_ROLE = {'бариста': 'бар', 'кассир': 'касса', 'повар': 'кухня',
                'старший смены': 'кухня', 'старший повар': 'кухня',
                'заготовщик': 'цех',
                'заготовщик — лаваши': 'цех',
                'заготовщик — мясо и соусы': 'цех',
                'заготовщик — выпечка и десерты': 'цех',
                'уборщица': 'зал',
                'закупщик': 'цех', 'управляющий': 'управление'}


def welcome_text(name, point, role, dept):
    """Первое сообщение новому человеку: не «ты в системе», а что делать.

    Раньше приходила одна строка «напиши меню». Человек открывал меню
    и не понимал порядка: почему не открывается рабочее место, зачем
    геолокация, что делать на пересменке. Правка Азиза 30.08.
    """
    # Одиночки — цех и управляющий — смену дня не выбирают и смену
    # не передают: они на точке одни и отвечают за весь день.
    alone = dept == 'цех' or DEPT_BY_ROLE.get(str(role).strip().lower()) == 'цех' \
        or str(role).strip().lower() == 'управляющий'
    zone = ('\nТвоя зона закреплена — выбирать не нужно.'
            if C.station_for(role) else '')
    steps = ['<b>Приход.</b> Нажми «Пришёл» и разреши доступ к месту — '
             'без геометки приход не запишется. Уход отмечается так же.']
    if not alone:
        steps.append('<b>Смена.</b> Выбери: первая — открываешь точку '
                     'и передаёшь сменщику; вторая — принимаешь и закрываешь.')
    if not C.station_for(role):
        steps.append('<b>Рабочее место.</b> Займи своё — откроются '
                     'только твои листы.')
    steps.append('<b>Чек-листы.</b> ✓ — сделано, ✗ — нет, и тогда обязательно '
                 'напиши почему. Где просят снимок — фотографируй на месте, '
                 'из галереи не примет.')
    if alone:
        steps.append('<b>Открытие и закрытие.</b> Ты на своём месте один: '
                     'сам открываешь смену и сам закрываешь её, когда '
                     'выполнил объём. Передавать некому.')
    else:
        steps.append('<b>Пересменка.</b> Уходишь — сдай смену и выбери, кому '
                     'передаёшь. Заступаешь — прими и проверь своими глазами.')
    steps.append('<b>Уход.</b> Нажми «Ушёл» и выбери: домой, на другую точку '
                 'или остаёшься здесь на другом месте.')
    body = '\n'.join(f'{i}. {t}' for i, t in enumerate(steps, start=2))
    return (
        f'Привет, {name}! Тебя добавили в систему «Ромашки».\n'
        f'<b>{point} · {role}</b>' + (f' · {dept}' if dept else '') + zone
        + '\n\n<b>Как работать — по шагам:</b>\n'
        '1. Открой приложение кнопкой ниже.\n' + body + '\n\n'
        '<b>Чего делать нельзя:</b> ставить ✓ не глядя, снимать фото '
        'заранее или из галереи, уходить не отметив уход — часы тогда '
        'не посчитаются.\n\n'
        'Не работает или непонятно — пиши управляющему, не гадай.')


def add_done(boss, add):
    dept = DEPT_BY_ROLE.get(add['role'].strip().lower(), '')
    S.add_member(add['id'], add['name'], add['point'], add['role'], dept)
    say(boss, f'✅ Добавил.\n<b>{add["name"]}</b> · {add["point"]} · {add["role"]}'
              + (f' · {dept}' if dept else
                 '\n⚠️ Позиция не определилась — проставь отдел в «Команде», '
                 'иначе чек-листы ему не откроются.'))
    kb = ({'inline_keyboard': [[{'text': '📱 Открыть приложение',
                                 'web_app': {'url': C.WEBAPP_URL}}]]}
          if C.WEBAPP_URL else None)
    r = say(add['id'], welcome_text(add['name'], add['point'], add['role'], dept),
            **({'reply_markup': kb} if kb else {}))
    # Бот не может написать первым тому, кто его не запускал. Молчать об этом
    # нельзя: человек будет ждать инструкцию, которая не придёт.
    if not (r or {}).get('ok'):
        say(boss, f'⚠️ <b>{add["name"]}</b> не получил инструкцию: он ещё '
                  'не запускал бота. Скажи ему открыть бота и нажать «Старт» — '
                  'после этого пришлю ещё раз командой «инструкция ' 
                  + str(add['id']) + '».')
...
def on_message(msg):
    ...
    # Повторная отправка инструкции: бот не может написать первым тому,
    # кто его не запускал. Человек нажал «Старт» — руководитель шлёт снова.
    if low.startswith('инструкция') and not st:
        who = S.team().get(chat_id)
        if not who or S.role_of(who) not in ('manager', 'coo'):
            say(chat_id, 'Инструкцию отправляет руководитель.')
            return True
        target = t.split()[-1].strip()
        v = S.team().get(target)
        if not v:
            say(chat_id, 'Не нашёл такого в «Команде». Формат: '
                         '<code>инструкция 123456789</code>')
            return True
        kb = ({'inline_keyboard': [[{'text': '📱 Открыть приложение',
                                     'web_app': {'url': C.WEBAPP_URL}}]]}
              if C.WEBAPP_URL else None)
        r = say(target, welcome_text(v[0], v[1], v[2] if len(v) > 2 else '',
                                     S.dept_of(v)),
                **({'reply_markup': kb} if kb else {}))
        say(chat_id, f'✅ Инструкция отправлена: {v[0]}' if (r or {}).get('ok')
            else f'⚠️ {v[0]} всё ещё не запускал бота — сообщение не дошло.')
        return True
