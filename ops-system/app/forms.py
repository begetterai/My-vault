...
def mark_shift(direction, day, point, who, lat, lon, plan=None, photo='',
    ...
    Под замком целиком: здесь только работа с таблицей — прочитали, есть ли
    открытая смена, и записали. Раньше замок висел на всём обработчике
    приложения, вместе с отправкой сообщений и загрузкой фото, и очередь
    из-за одного человека с тремя снимками растягивалась на минуты.
