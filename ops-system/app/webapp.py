...
def init_payload(who):
    ...
    try:
        read = S.read_codes(who[0])
    except Exception as e:
        print('ознакомления:', e)
        read = set()
    out['quizzes'] = [{'key': k, 'title': cl['title'], 'code': cl['code'],
                       'read': (cl.get('doc') or {}).get('code', '') in read,
                       'icon': cl.get('icon', '🎓'), 'doc': cl.get('doc'),
    ...
    # Кому можно сдать смену. Директор из списка убран: он смену не принимает
    # и на месте не стоит — его дело проверить, что её приняли правильно.
    out['mates'] = sorted({v[0] for v in S.team().values()
                           if v[0] != who[0] and S.role_of(v) != 'coo'})
    ...
    # Руководителю — оглавление всех чек-листов сети: он их не заполняет,
    # но должен видеть, по чему работают люди, и править формулировки.
    # Управляющий здесь наравне с директором: неточность в пункте первым
    # замечает тот, кто каждый день ходит по точке.
...
def read_doc(who, body):
    ...
    Регламент читается до объяснения и теста: иначе обучение превращается
    в заучивание вопросов, а первоисточник так никто и не открывает.
    """
...
# Разрешение на один раз, но живёт оно в таблице, а не в памяти:
# 14.09.2026 выяснилось, что выкатка гасит выданные разрешения, и человек
# упирается в ту же стену после того, как управляющий уже нажал «Разрешить».
GRANT_MIN = 30
...
def leave_allowed(name):
    ...
    Смотрим, не гасим. Раньше это было гасящее чтение, и вопрос «а можно
    уже уйти?» сам съедал разрешение: leave_ask видел пустую причину
    и отвечал «уже можно уходить», а «Ушёл» после этого упирался в блок.
    Гасит его shift() при настоящем уходе.
    """
    return S.peek_grant(name, 'уход')
...
def geo_allowed(name):
    ...
    Смотрим, не гасим: разрешение может и не понадобиться, если место
    подтвердилось само. Гасит его shift(), когда оно действительно
    что-то пропустило.
    """
    return S.peek_grant(name, 'геометка')
...
def shift(who, body):
    ...
    lat, lon = body.get('lat'), body.get('lon')
    point = pick_point(who, body)
    # Разрешение управляющего спрашиваем до всех проверок места.
    # 14.09.2026 на ОВИР человек упёрся в стену: телефон не отдавал
    # координат вовсе, управляющий разрешение дал — а проверка «нет
    # координат» стояла выше и обрывала отметку, не дойдя до разрешения.
    # Разрешили, перезапустили, и та же ошибка. Разрешение обязано
    # перекрывать любую причину, иначе оно не разрешение.
    allow = geo_allowed(who[0])
    if lat is None and not allow:
        ...
        # far обязателен: по нему приложение показывает кнопку «Я на месте —
        # попросить управляющего». Без него человек в подвале или в цехе
        # без окон оставался заперт — приход не отмечен, значит закрыты
        # и рабочее место, и все листы, а обойти изнутри нечем. Запрос
        # уходит и без координат: geo_ask пишет управляющему «места нет».
        return {'ok': False, 'far': True, 'point': point,
                'error': 'Не вижу, где ты. Разреши доступ к геолокации '
                         'и нажми ещё раз. Если место не определяется — '
                         'попроси управляющего подтвердить, что ты на точке.'}
    ...
    if want != point and want in S.points():
        ...
        # Проверяем только когда есть чем: без координат «не там» сказать
        # нельзя, а с разрешением управляющего человек сюда и доходит.
        if lat is not None and F.geo_check(want, lat, lon)[1] and not allow:
            return {'ok': False,
    ...
    if d == 'out':
        ...
        left_by = leave_allowed(who[0])
        if left_by:
            left_note = f'уход разрешил {left_by}'
    ...
    try:
        ...
        # времени, придя на вечернюю смену вовремя. А он её почти никогда
        # не выбирает: селектор смены в приложении появляется только ПОСЛЕ
        # отметки прихода. Поэтому передаём время прихода — по нему
        # выбирается ближайшая строка состава.
        came = str(body.get('at') or '').strip() or C.now().strftime('%H:%M')
        plan = RS.start_of(C.today(), who[0], plan, part=part, at=came)
    ...
    # Отметка прошла — вот теперь гасим разрешения, и только те, что реально
    # что-то пропустили. Раньше они забирались первой строкой обработчика
    # на каждой отметке: поймался GPS — разрешение съедено молча, и вечером
    # управляющего просили второй раз.
    if allow and (far or lat is None):
        S.take_grant(who[0], 'геометка')
    if left_by:
        S.take_grant(who[0], 'уход')
    if d == 'out':
...
def faults(key, line, who, body):
    ...
    try:
        ...
        day, point, filler = ((r[0][0], r[0][1], r[0][2])
                              if r and len(r[0]) > 2 else ('', who[1], ''))
    except Exception:
        day, point, filler = '', who[1], ''
    texts = {str(n): t for n, _b, t in C.flat(key)}
    # День берём из самого листа, а не сегодняшний. ОВИР закрывается в 03:30,
    # лист ложится в операционные сутки прошлого дня, а разбирают его утром
    # следующего — и минус за невыполненный пункт уходил тому, кто в ту ночь
    # не работал. На ночных точках это происходило всегда.
    one, two = shift_people(key, day or C.day_str(), point, filler)
    for n in first:
        if one:
