...
def geo_allowed(name):
    ...
    return S.take_grant(name, 'геометка')
...
def shift(who, body):
    ...
    if d == 'out':
        stop, can_ask = leave_blocked(who, point)
        if stop:
            # Разрешение на геометку уже забрали, а до ухода дело не дошло.
            # Возвращаем, иначе оно сгорит впустую.
            if far and allow:
                S.grant(who[0], 'геометка', allow, GRANT_MIN)
    ...
    if not saved:
        # Разрешение мы уже забрали, а отметки не вышло. Возвращаем: человек
        # не виноват, что нажал не ту кнопку, и второй раз просить
        # управляющего из-за этого не должен.
        for kind, by in (('геометка', allow if far else ''),
                         ('уход', left_by)):
            if by:
                S.grant(who[0], kind, by, GRANT_MIN)
