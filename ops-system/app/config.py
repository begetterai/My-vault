...
ROLLOUT = [x.strip() for x in ENV('ROLLOUT', 'coo,manager,senior').split(',') if x.strip()]
...
# Напоминания о дедлайне и «просрочено». На обкатке выключены: листов смены
# двенадцать, и в 10:00 управляющий получил бы двенадцать красных сообщений
# в первый же день. Дедлайны при этом остаются — по ним группируется
# «Сегодня» в приложении.
REMINDERS = ENV('REMINDERS', '').strip().lower() in ('1', 'on', 'да', 'true')
