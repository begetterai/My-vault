...
RULES = {
    # базовый счёт — своя ежедневная работа. Правка Азиза 12.09.2026:
    # за день человек получает ровно одну пятёрку, и только если сделал
    # своё сам И управляющий это подтвердил. Отдельного начисления
    # за подтверждённый лист больше нет: два начисления за одну и ту же
    # работу человек всё равно считал одним.
    'day_closed':     (5,   BASE,  'День закрыт полностью'),
    'fill_late':      (-5,  BASE,  'Чек-лист сдан позже срока'),
    'item_fail':      (-10, BASE,  'Невыполненный пункт — вина смены'),
    'mismatch':       (-10, BASE,  'Расхождение при проверке управляющим'),
    'late':           (-1,  BASE,  'Опоздание, за минуту'),
    # доп. счёт — сверх обязанностей. Замены, идеи и обучение начисляет
    # управляющий руками (AWARDABLE); находка — сама, по факту записи
    # (решение Азиза 31.08.2026).
    # Сданный тренинг убран 12.09.2026: обучение — обязанность и допуск
    # к работе, а не подвиг. Обучить новичка — по-прежнему сверх.
    'found_issue':    (5,   EXTRA, 'Нашёл и записал проблему'),
    'idea_accepted':  (20,  EXTRA, 'Идея принята в работу'),
    'replace_shift':  (5,   EXTRA, 'Вышел на замену за другого'),
...
def close_day(d=None):
    """Раз в сутки: кто закрыл своё и чьё подтвердили — тому +5.

    Считается по факту, а не по обещанию: берём людей, отметивших приход,
    и смотрим, сданы ли чек-листы их отдела за этот день и подтверждены ли.

    Вызывается повторно, когда подтверждение пришло уже после закрытия
    суток, — от дубля защищает проверка ниже.
    """
    d = d or C.today()
    ...
    def confirmed_own(point, who):
        """Подтвердил ли управляющий всё, что человек сдал за день.

        Правка Азиза 12.09.2026: пятёрка одна, и она даётся только когда
        человек сделал своё сам И это подтвердили. Раньше подтверждение
        было отдельным начислением, и за день с тремя листами набегало
        двадцать баллов — люди всё равно считали это «пятёркой за день».
        """
        got = seen.get(point) or {}
        mine = [v for v in got.values() if v.get('who') == who]
        return bool(mine) and all(v.get('ok') for v in mine)
