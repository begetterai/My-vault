...
def close_day(day=None):
    ...
    День принимаем аргументом: догон (строка ниже по файлу) зовёт
    close_day(prev) за вчера. Раньше функция аргументов не принимала —
    догон падал с TypeError, ключ once() был уже израсходован, и повтора
    не случалось никогда: за те сутки +5 не получал никто.
    """
    from . import score as SC
    try:
        n = SC.close_day(day or C.today())
        if n:
